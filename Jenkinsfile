...
def buildFPGAKernels(params, String name, String suffix) {
    ...
    stage("Building FPGA kernels") {
        cheribuildProject(target: "cheribsd-${suffix}", architecture: suffix,
            extraArgs: "--cheribsd/build-options=-s --cheribsd/no-debug-info --keep-install-dir --install-prefix=/rootfs --cheribsd-${suffix}/pure-cap-kernel --cheribsd-${suffix}/build-fpga-kernels --cheribsd-${suffix}/mfs-root-image $WORKSPACE/cherisdk/disk-image-minimal-${suffix} --skip-buildworld",
